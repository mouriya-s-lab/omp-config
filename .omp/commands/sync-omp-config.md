...
| `$AGENT_DIR/thinking-translator.json` | `agent/thinking-translator.json` |
...
三个 light 源资产只从 `$AGENT_DIR` 同步。绝不扫描、复制或提交安装目录（`dirname(Bun.which("omp"))`）中的 `omp-light`、`omp-light.ts` 或 `omp-light.cmd`；这些是机器上的已安装入口，不属于仓库快照。此命令也不安装启动器、不 chmod、不修改 PATH 或任何 shell rc/profile。
...
1. 先解析 `$AGENT_DIR` 为绝对路径，再逐文件比对本机与仓库，列出“仓库缺失 / 内容不同 / 本机已删除”三类差异；差异清单必须包含 `$AGENT_DIR/config-light.yml`、`$AGENT_DIR/APPEND_SYSTEM_LIGHT.md`、`$AGENT_DIR/omp-light.ts`。三个 light 源资产从 `$AGENT_DIR` 读取，安装目录中的 `omp-light`、`omp-light.ts`、`omp-light.cmd` 不进入扫描；无差异就直接报告已同步，不做多余改动。
...
4. 校验：普通同步文件（含三个 light 源资产与 `thinking-translator.json`）与源文件 `cmp` 一致；安装目录的任何启动器都不参与校验；`config.yml` 只比较未排除字段，排除字段不参与同步校验；`bun -e` 能分别用 `Bun.YAML.parse` 解析 `$AGENT_DIR/config-light.yml` 和 `agent/config-light.yml`，并能解析 `agent/config.yml`（`Bun.YAML.parse`）、`agent/settings.json` 和 `agent/thinking-translator.json`（`JSON.parse`）。

...
在仓库根目录启动 OMP，执行仓库级命令（定义见 `.omp/commands/migrate-omp-keys.md`），把本机 `~/.omp/agent/agent.db` 里的 `auth_credentials` 记录经 SSH 传到指定远端 OMP 主机：
...
当前定义的职责分层与共同拆分规则如下。所有 `task:*` 都是 Opus 级别模型上的通用执行 agent。`task:free`、`task:low`、`task:mid` 的 `description` 明确禁止交给它们任何设计和核心工作（架构、领域类型与状态模型、接口与跨切片契约、改动的核心逻辑，以及文档、prompt、skill、agent 定义的设计），它们只承担此外的调查、实现、调试、拆分和验证；`task:high` 的 `description` 不设这条限制。在各自允许的范围内，tier 只表示模型成本与单次结果的预期可信度，按结果需要的可信度与验证代价选择，不按任务难度或歧义程度选择。
...
| 类型 | 名称 | 设计职责 |
| --- | --- | --- |
| 执行 | `task:high` | 通用顶级 tier，模型为 Anthropic Claude Opus 5.5，地球上最好的模型，基本完美无缺；综合输入、输出、缓存等各类 token，每 1M token 约 0.45 USD；`description` 不设设计和核心工作的禁令，用于结果必须一次做对，或包括 `task:mid` 在内的便宜 tier 都无法裁决的情况。 |
| 执行 | `task:mid` | 通用中等成本 tier，综合输入、输出、缓存等各类 token，每 1M token 约 0.3 USD，大约与 GLM 5.2 相当；不接任何设计和核心工作，用于此外错误代价高、证据难以取得或便宜模型相互冲突的工作，购买比 low/free 更强的预期判断力与可信度。 |
| 执行 | `task:low` | 通用低成本 tier，综合输入、输出、缓存等各类 token，每 1M token 约 0.01 USD，比 DeepSeek 任何版本都便宜；不接任何设计和核心工作，此外的结果可信到可以直接交付，是委派工作的默认 tier；free 结果交给 subagent 验证时也由它担任验证者，独立复现关键检查、用实际产物和 runtime 证据核验 free 的结论并裁决冲突。 |
| 执行 | `task:free` | 通用零成本 tier，几乎可无限并发；不接任何设计和核心工作，且单个结果低可信，只用于结果无需独立验证的工作：定位候选代码或文档、列举方案或假设、只影响下一步方向的探索性试验，错误无害或会在调用方下一步自然暴露。验证计入成本：需要验证才能采信的结果，free 加验证者比 `task:low` 一次做完更贵，所以落进仓库的改动、会被直接采信的结论和裁决交给 `task:low` 及以上。free 结果不能相互验证；意外成为关键结论时，仍由主 agent 或不参与产出的 `task:low` 及以上 tier 独立验证。 |
...
- `agent/thinking-translator.json`：思考过程翻译配置（`omp-thinking-translator` 读 `~/.omp/agent`）
...
解压后必须存在 `${pty_root}/build/Release/pty.node` 和 `${pty_root}/build/Release/spawn-helper`。缓存位置是 `~/.omp/unified-exec-bun-pty-binding/<包版本>-darwin-arm64/`。`.ts` 扩展文件仍按上面的本地扩展迁移规则放在 `~/.omp/agent/extensions/`，不要把 PTY 归档解到那里。
